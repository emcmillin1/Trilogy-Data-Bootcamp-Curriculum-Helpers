...
class Square():
    ...
    def __init__(self, position, last_square):
        ...
        self.last_square = (last_square==position)
        self.type='square'

    def land_on(self, player):
        print('{} landed on {}'.format(player.name, self.position))
    ...
    def board_end(self):
        print('{} is the last square! You win!'.format(self.position))
...
class Chute(Square):
    ...
    def __init__(self, position, end, last_square):
        self.position = position
        self.end = end
        self.last_square = (last_square==end)
        self.type = 'chute'

    def land_on(self,player):
        print('{} landed on {}'.format(player.name, self.position))
        print('Oh No! {} is a Chute!... {} slides down to {}'.format(self.position, player.name, self.end))
        player.position = self.end
        if self.last_square:
            self.board_end()
            return True
        else:
    ...
    def board_end(self):
        print('{} is the last square! You win!'.format(self.end))
...
class Ladder(Square):
    ...
    def land_on(self,player):
        print('{} landed on {}'.format(player.name, self.position))
        print('Hooray! {} is a Ladder!... {} climbs to {}'.format(self.position, player.name, self.end))
        player.position = self.end
        if self.last_square:
            self.board_end()
            return True
...
class Board():
    ...
    def __init__(self, chutes='default', ladders='default', n_squares=100):
        ...
        for i in range(1,n_squares+1):
            if i in chutes.keys():
                self.squares[i] = Chute(position=i, end=chutes[i], last_square=n_squares)
            elif i in ladders.keys():
                self.squares[i] = Ladder(position=i, end=ladders[i], last_square=n_squares)
            else:
                self.squares[i] = Square(position=i, last_square=n_squares)

        self.cell_colors = {'square':'6;30;47','chute':'7;31;40','ladder':'7;32;40'}
        # self.base_str_format = '\x1b[{style};{bg};{fg}m {text} \x1b[0m'
        self.base_str_format = '\x1b[{style}m {text} \x1b[0m'
        self.square_inhabitants = defaultdict(list)


    def reference_square(self, player, position):
        if position>=self.n_squares:
            print('Well Done {}, you\'ve reached the end of the board!'.format(player.name))
            return True

        else:
            # executes squares land on method, returns True if player won, else False
            self.square_inhabitants[position].append(player.name)
            return self.squares[position].land_on(player)
